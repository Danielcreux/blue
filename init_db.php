...
$stmt->execute([':username' => 'danielcreux']);
if ($stmt->fetchColumn() == 0) {
    $hashedPassword = password_hash('danielcreux', PASSWORD_DEFAULT);
    $stmt = $db->prepare("INSERT INTO users (username, name, email, password) VALUES (:username, :name, :email, :password)");
    $stmt->execute([
        ':username' => 'danielcreux',
        ':name' => 'Joshué Daniel Freire Sánchez',
        ':email' => 'info@freire-sanchez-valencia.es',
